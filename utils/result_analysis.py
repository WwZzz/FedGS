...
class Drawer(Analyser):
        ...
        for func in ploter:
            if hasattr(self, func):
                ...
                for plot_obj in ploter[func]:
                    tmp = self.colors
                    if 'colors' in plot_obj:
                        self.colors = plot_obj['colors']
                    elif 'color' in plot_obj or 'c' in plot_obj:
                        s = 'color' if 'color' in plot_obj else 'c'
                        self.colors = [plot_obj[s] for _ in range(1000)]
    ...
    def vir_bar(self, plot_obj):
        ...
        for id,rec in enumerate(self.records):
            ...
            x = dict[plot_obj['x']]
            y = dict[plot_obj['y']]
            ax.bar(x,y,label=dict['legend'],color=self.colors[id])
            ax.set_ylim([0,200])
            ax.set_xlim([-0.5,len(x)])
            handles.append(mpatches.Patch(color=self.colors[id], label=dict['legend']))
            # ax.set_title(dict['legend'])
        plt.gca().margins(x=0)
        plt.gcf().canvas.draw()
        ...
        # plt.legend(handles=handles,  ncol=len(self.records), loc='upper center')
        plt.tight_layout()
        # plt.legend()
        return
    ...
    def group_plot(self, plot_obj):
        for id, item in enumerate(self.grouped_records.items()):
            ...
            if 'label' in plot_obj.keys():
                ax.plot(x, mean_val, color=self.colors[id], label=plot_obj['label'][id])
            else:
...
class Former(Analyser):
    ...
    def group_testing_optimal_value(self, key):
        res = []
        for id, item in enumerate(self.grouped_records.items()):
            group_name, rec_dicts = item
            # find the round of the best result on validation dataset
            op_rounds = [np.argmin(rec['valid_loss']) for rec in rec_dicts]
            testing = [rec[key][r] for rec,r in zip(rec_dicts, op_rounds)]
            print(op_rounds)
            if 'accuracy' in key:
                testing = np.array(testing)*100
            mean_val = np.mean(testing)
            std_val = np.std(testing)
            res.append("{:.4f}".format(mean_val)+'±'+"{:.4f}".format(std_val))
        self.group_tb.add_column(fieldname='optimal_'+key, column=res)
        return
        res = []
        for id, item in enumerate(self.grouped_records.items()):
            group_name, rec_dicts = item
            mean_val = statistic_on_dicts(rec_dicts, name='min', key=key)[-1]
            std_val = statistic_on_dicts(rec_dicts, name='std', key=key)[-1]
            res.append("{:.2f}".format(mean_val)+'±'+"{:.2f}".format(std_val))
        self.group_tb.add_column(fieldname=key, column=res)

    def group_mean_of_min_value(self, key):
        res = []
        for id, item in enumerate(self.grouped_records.items()):
            group_name, rec_dicts = item
            tmp = np.array([min(d[key]) for d in rec_dicts]).mean()
            res.append("{:.3f}".format(tmp))
        self.group_tb.add_column(fieldname="min_"+key, column=res)
